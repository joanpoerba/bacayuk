...
const AdminPetugas = () => {
  ...
  const getUsers = async () => {
    const response = await axios.get("http://localhost:3000/admin/petugas");
    setPetugas(response.data);
  };

  const deleteUser = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/admin/hapuspetugas/${id}`);
      location.reload();
      getUsers();
    } catch (error) {
      console.log(error);
      ...
      <Flowbite theme={{ theme: adminOverviewCustomTheme }}>
        <div className="mt-9 pb-10">
          ...
          <div className="overflow-x-auto mt-5">
            <Table hoverable className="w-[1000px] lg:w-full">
              <Table.Head>
